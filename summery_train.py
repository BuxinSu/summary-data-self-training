...
# Parameters
n_values = [2500, 20000, 40000, 60000]  # Number of samples
p = 5000  # Number of features
kappa_values = [0.05, 0.1, 0.15, 0.2]  # Probability that a coordinate of beta is zero
...
noise_std_values = [0.5, 1, 1.5]  # Standard deviation of the noise
...
for n in n_values:
    for kappa in kappa_values:
        for noise_std in noise_std_values:
            ...
            plt.legend()
            plt.grid(True)
            filename = f'/path/to/summary_training/summery_n{n}_p_4k6_kappa{kappa}_sigmabeta_5_sigmanoise{noise_std}.png'
            plt.savefig(filename)
            plt.show()

            # Save the R_squared_sum_values to a text file
            txt_filename = f'/path/to/summary_training/R_squared_sum_n{n}_p_4k6_kappa{kappa}_sigmabeta_5_sigmanoise{noise_std}.txt'
            np.savetxt(txt_filename, R_squared_sum_values)

            # Save the R_squared_sum_values to a text file
            txt_filename = f'/path/to/summary_training/R_squared_ind_n{n}_p_4k6_kappa{kappa}_sigmabeta_5_sigmanoise{noise_std}.txt'
