...
def read_shrinkage_matrix(p, df, lam):
    lam_formatted = f"{lam:.4f}"
    filepath = f'/path/to/summary_training/reference_panels_heavy_tail/reference_panel_heavy_tail_p{p}_nw1000_df{df}_lam{lam_formatted}.csv'
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Shrinkage matrix file not found: {filepath}")
    return pd.read_csv(filepath, header=None).values

def main():
    parser = argparse.ArgumentParser(description='Run heavy-tail simulation with specified kappa, noise_std, iteration, p, n, and df.')
    ...
    print(f"n_sum_total={n_sum_total}, n_sum_train={n_sum_train}, n_sum_valid={n_sum_valid}, df={df}")
    ...
    for lam in lambdas:
        Shrinkage = read_shrinkage_matrix(p, df, lam)
    ...
    results_sum_filename = f'results_sum_heavy_tail_n{n}_p{p}_df{df}_kappa{kappa}_noise{noise_std}_iter{iteration}.csv'
    ...
    results_ind_filename = f'results_ind_heavy_tail_n{n}_p{p}_df{df}_kappa{kappa}_noise{noise_std}_iter{iteration}.csv'
