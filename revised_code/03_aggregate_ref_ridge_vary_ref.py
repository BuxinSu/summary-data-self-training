...
def parse_filename(filepath):
    """
    Extract n, p, nw, kappa, noise from filename.
    Example: results_sum_n12500_p5000_nw1000_kappa0.9_noise2.0_iter4.csv
    Returns: {'n': 12500, 'p': 5000, 'n_w': 1000, 'kappa': 0.9, 'noise_std': 2.0}
    """
...
def main():
    parser = argparse.ArgumentParser(description='Aggregate ref_ridge vary_ref results over iterations.')
    ...
    # Average R_squared_ind over iterations for each (n, n_w, p, kappa, noise_std, lambda)
    group_cols = ['n', 'n_w', 'p', 'kappa', 'noise_std', 'lambda']
    ...
    # Average R_squared_sum over iterations for each (n, n_w, p, kappa, noise_std, lambda)
    ...
    # Reorder columns
    merged = merged[['n', 'n_w', 'p', 'kappa', 'noise_std', 'lambda', 'R_squared_ind', 'R_squared_sum']]
    merged.sort_values(by=['n', 'n_w', 'p', 'kappa', 'noise_std', 'lambda'], inplace=True)

    # ---- Save ----
    output_filepath = os.path.join(output_dir, 'aggregated_ref_ridge_vary_ref.csv')
