...
def main():
    parser = argparse.ArgumentParser(description='Test heavy-tail ref_ridge models using best lambdas from aggregated results.')
    parser.add_argument('--aggregated_csv', type=str,
                        default='/path/to/summary_training/results/ref_ridge_heavy_tail/aggregated_ref_ridge_vary_ref_heavy_tail.csv',
                        help='Path to aggregated CSV with averaged R_squared_ind and R_squared_sum')
    parser.add_argument('--data_dir', type=str,
                        default='/path/to/summary_training/Individual_Data_heavy_tail',
                        help='Directory containing heavy-tail X, y, beta CSVs')
    ...
    # Group by (n, n_w, p, df, kappa, noise_std) and find lambda that maximizes each R²
    group_cols = ['n', 'n_w', 'p', 'df', 'kappa', 'noise_std']
    ...
    for _, row in best_lams.iterrows():
        ...
        print(f"\nSetting: n={n}, n_w={n_w}, p={p}, df={df}, kappa={kappa}, noise_std={noise_std}")
        print(f"  lam_ind={lam_ind:.4f}, lam_sum={lam_sum:.4f}")

        # Load heavy-tail X and y
        x_filepath = os.path.join(args.data_dir, f"X_heavy_tail_n{n}_p{p}_df{df}_kappa{kappa}_noise{noise_std}.csv")
        y_filepath = os.path.join(args.data_dir, f"y_heavy_tail_n{n}_p{p}_df{df}_kappa{kappa}_noise{noise_std}.csv")
        ...
        Shrinkage_sum = read_shrinkage_matrix(p, n_w, df, lam_sum)
        ...
        Shrinkage_ind = read_shrinkage_matrix(p, n_w, df, lam_ind)
    ...
    output_filepath = os.path.join(args.output_dir, 'test_results_ref_ridge_vary_ref_heavy_tail.csv')
