...
RESULTS_DIR = Path(
    os.environ.get("LDPRED2_RESULTS_DIR", BASE_DIR / "Results" / "Summery_Validation")
).expanduser()
FIGURES_DIR = Path(
    os.environ.get("LDPRED2_FIGURES_DIR", BASE_DIR / "Figures")
).expanduser()
...
def read_r_squared(path: Path) -> pd.Series:
    df = pd.read_csv(path)
    if "r_squared" not in df.columns:
        raise ValueError(f"{path} does not contain an 'r_squared' column")

    return pd.to_numeric(df["r_squared"], errors="coerce").dropna()
...
def build_pair_table(
    ...
    for trait in sorted(ind_traits):
        ind_values = read_r_squared(ind_files[trait])
        sum_values = read_r_squared(sum_files[trait])
...
def plot_top_pair_boxplots(
    ...
    sum_sd_col = f"{sum_label}_SD"
    top_df = (
        table.nlargest(TOP_N, "pair_max_SD")
        .sort_values(sum_mean_col, ascending=False, kind="mergesort")
    )
    ...
    print(f"\nTop {TOP_N} boxplot pairs for {comparison_label}:")
    print(
        top_df[
            [
                "trait",
    ...
    print(f"Saved paired boxplot to: {output_file}")
...
def main() -> None:
    ...
    plot_top_pair_boxplots(
        ...
        comparison_label="LDpred2_ind vs LDpred2_sum",
        sum_label="LDpred2_sum",
        output_file=FIGURES_DIR / "LDpred2_ind_vs_sum_top10_sd_paired_boxplot.pdf",
    )

    val_100_table = build_pair_table(
    ...
    plot_top_pair_boxplots(
        ...
        comparison_label="LDpred2_ind vs LDpred2_sum_val_100",
        sum_label="LDpred2_sum_val_100",
        output_file=FIGURES_DIR / "LDpred2_ind_vs_sum_val_100_top10_sd_paired_boxplot.pdf",
    )
