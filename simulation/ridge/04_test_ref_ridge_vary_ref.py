...
def read_shrinkage_matrix(p, n_w, lam):
    lam_formatted = f"{lam:.4f}"
    filepath = f'/path/to/summary_training/reference_panels/reference_panel_p{p}_nw{n_w}_lam{lam_formatted}.csv'
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Shrinkage matrix file not found: {filepath}")
    return pd.read_csv(filepath, header=None).values

def main():
    parser = argparse.ArgumentParser(description='Test ref_ridge models using best lambdas from aggregated results.')
    parser.add_argument('--aggregated_csv', type=str,
                        default='/path/to/summary_training/results/ref_ridge/aggregated_ref_ridge_vary_ref.csv',
                        help='Path to aggregated CSV with averaged R_squared_ind and R_squared_sum')
    parser.add_argument('--data_dir', type=str,
                        default='/path/to/summary_training/Individual_Data',
                        help='Directory containing X, y, beta CSVs')
    parser.add_argument('--output_dir', type=str,
                        default='/path/to/summary_training/results/ref_ridge',
    ...
    # Group by (n, n_w, p, kappa, noise_std) and find lambda that maximizes each R²
    ...
    for _, row in best_lams.iterrows():
        ...
        print(f"\nSetting: n={n}, n_w={n_w}, p={p}, kappa={kappa}, noise_std={noise_std}")
        print(f"  lam_ind={lam_ind:.4f}, lam_sum={lam_sum:.4f}")

        # Load X and y
        x_filepath = os.path.join(args.data_dir, f"X_n{n}_p{p}_kappa{kappa}_noise{noise_std}.csv")
        y_filepath = os.path.join(args.data_dir, f"y_n{n}_p{p}_kappa{kappa}_noise{noise_std}.csv")
        ...
        Shrinkage_sum = read_shrinkage_matrix(p, n_w, lam_sum)
        ...
        Shrinkage_ind = read_shrinkage_matrix(p, n_w, lam_ind)
    ...
    output_filepath = os.path.join(args.output_dir, 'test_results_ref_ridge_vary_ref.csv')
