...
def read_shrinkage_matrix(p, lam):
    ...
    filepath = f'/path/to/summary_training/reference_panels/reference_panel_p{p}_nw1000_lam{lam_formatted}.csv'
    if not os.path.exists(filepath):
        raise FileNotFoundError(f"Shrinkage matrix file not found: {filepath}")
    return pd.read_csv(filepath, header=None).values

def main():
    parser = argparse.ArgumentParser(description='Run simulation with specified kappa, noise_std, iteration, p, and n.')
    ...
    print(f"n_sum_total={n_sum_total}, n_sum_train={n_sum_train}, n_sum_valid={n_sum_valid}")
    ...
    results_sum_filename = f'results_sum_n{n}_p{p}_kappa{kappa}_noise{noise_std}_iter{iteration}.csv'
    ...
    results_ind_filename = f'results_ind_n{n}_p{p}_kappa{kappa}_noise{noise_std}_iter{iteration}.csv'
