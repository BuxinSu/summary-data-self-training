...
def main():
    parser = argparse.ArgumentParser(description='Run simulation with specified kappa, noise_std, iteration, p, n, and n_w.')
    ...
    print(f"n_sum_total={n_sum_total}, n_sum_train={n_sum_train}, n_sum_valid={n_sum_valid}, n_w={n_w}")

    # ---- Save directory ----
    save_dir = "/path/to/summary_training/results/ref_ridge"
    ...
    for lam in lambdas:
        Shrinkage = read_shrinkage_matrix(p, n_w, lam)
    ...
    results_sum_filename = f'results_sum_n{n}_p{p}_nw{n_w}_kappa{kappa}_noise{noise_std}_iter{iteration}.csv'
    ...
    results_ind_filename = f'results_ind_n{n}_p{p}_nw{n_w}_kappa{kappa}_noise{noise_std}_iter{iteration}.csv'
