...
def sample_random_state(path: Path, sample_n: int) -> int:
    seed_text = f"{RANDOM_SEED}:sample{sample_n}:{path.name}"
    digest = hashlib.sha256(seed_text.encode("utf-8")).hexdigest()
    return int(digest[:8], 16)


def sampled_r_squared_mean(path: Path, sample_n: int) -> float:
    df = pd.read_csv(path)
    ...
    sampled_values = values.sample(
        n=sample_n,
        replace=False,
        random_state=sample_random_state(path, sample_n),
    )
...
def build_sampled_mean_table(
    ...
    for trait in sorted(ind_traits):
        rows.append(
            {
                "trait": trait,
                "LDpred2_ind_mean": sampled_r_squared_mean(ind_files[trait], sample_n),
                f"{sum_label}_mean": sampled_r_squared_mean(sum_files[trait], sample_n),
            }
        )
...
def print_mean_table(
    ...
    print(f"\nAll sample-{sample_n} mean pairs: {comparison_label}")
    print(
...
def plot_mean_scatter(
    ...
    print(
        f"Saved sample-{sample_n} mean scatter plot for {comparison_label} to: "
        f"{output_file}"
    )
...
def run_for_sample_size(
    sample_n: int,
    ind_files: dict[int, Path],
    plain_sum_files: dict[int, Path],
    val_100_sum_files: dict[int, Path],
) -> None:
    ...
    print_mean_table("LDpred2_ind vs LDpred2_sum", plain_table, "LDpred2_sum", sample_n)
    plot_mean_scatter(
        table=plain_table,
        comparison_label="LDpred2_ind vs LDpred2_sum",
        sum_label="LDpred2_sum",
        output_file=FIGURES_DIR / f"LDpred2_ind_vs_sum_mean_sample{sample_n}_scatter.pdf",
    ...
    plot_mean_scatter(
        ...
        comparison_label="LDpred2_ind vs LDpred2_sum_val_100",
        sum_label="LDpred2_sum_val_100",
        output_file=FIGURES_DIR / f"LDpred2_ind_vs_sum_val_100_mean_sample{sample_n}_scatter.pdf",
...
def main() -> None:
    ...
    for sample_n in SAMPLE_SIZES:
        run_for_sample_size(sample_n, ind_files, plain_sum_files, val_100_sum_files)
